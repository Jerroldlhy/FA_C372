    ...
    if (form.matches('form[action^="/add-to-cart/"]')){
      ...
      fetch('/api/cart/add/' + id, {
        ...
        .then(data => {
          ...
          try {
            ...
            if (card && typeof data.remaining === 'number') {
              ...
              if (stockEl) {
                if (data.remaining > 0) {
                  stockEl.textContent = 'Seats: ' + data.remaining;
                } else {
                  stockEl.innerHTML = '<span class="badge bg-danger">Full</span>';
                }
              }
        ...
        .catch(err => {
          ...
          alert('Failed to add to learning cart');
        });
    }
  });
})();
...
(function(){
  ...
  style.textContent = '.ajax-toast{position:fixed;bottom:1.25rem;right:1.25rem;background:#0b4f6c;color:#fff;padding:.75rem 1rem;border-radius:.5rem;box-shadow:0 4px 12px rgba(0,0,0,.15);opacity:0;transform:translateY(10px);transition:.3s;} .ajax-toast.show{opacity:1;transform:translateY(0);}';
